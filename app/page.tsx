...
function loadDay(): DayData {
  ...
  const raw = localStorage.getItem('daily_' + TODAY_KEY())
  return raw ? JSON.parse(raw) : { tasks: [], notes: '' }
}

function saveDay(data: DayData) {
  localStorage.setItem('daily_' + TODAY_KEY(), JSON.stringify(data))
  const idx: string[] = JSON.parse(localStorage.getItem('daily_index') || '[]')
  if (!idx.includes(TODAY_KEY())) {
    idx.push(TODAY_KEY())
    localStorage.setItem('daily_index', JSON.stringify(idx))
  }
}
...
function addItem() {
  ...
  if (isAnchor) {
    if (timePicker?.value) {
      ...
      item.anchorH = h; item.anchorMin = min
      item.text = text
      timePicker.value = ''
    } else if (parsed) {
      ...
      item.text = text; item.anchorH = null; item.anchorMin = null
    }
...
function buildCSV() {
  ...
  state.tasks.forEach(t => {
    rows.push([dateKey, t.type, `"${t.text.replace(/"/g, '""')}"`, t.done ? 'yes' : 'no'])
  })
...
function exportCSV() {
  ...
  a.href = url
  a.download = `daily-${TODAY_KEY()}.csv`
  a.click()
...
function previewSummary() {
  ...
  if (dateEl) dateEl.textContent = TODAY_KEY()
  if (contentEl) contentEl.textContent = buildSummaryText()
...
function emailSummary() {
  const settings = JSON.parse(localStorage.getItem('daily_settings') || '{}')
...
function archiveToday() {
  const key = 'daily_archive_' + TODAY_KEY()
  localStorage.setItem(key, JSON.stringify({ date: TODAY_KEY(), tasks: state.tasks, notes: state.notes, exportedAt: new Date().toISOString() }))
  const idx: string[] = JSON.parse(localStorage.getItem('daily_archive_index') || '[]')
  if (!idx.includes(TODAY_KEY())) {
    idx.unshift(TODAY_KEY())
    localStorage.setItem('daily_archive_index', JSON.stringify(idx))
  }
  renderArchive()
}
...
function renderArchive() {
  ...
  if (!el) return
  if (!idx.length) {
    el.innerHTML = '<div class="empty-state">No archived days yet. Export today\'s summary to start your archive.</div>'
    return
  }
  el.innerHTML = idx.map(dateKey => {
    const raw = localStorage.getItem('daily_archive_' + dateKey)
    if (!raw) return ''
    const d = JSON.parse(raw)
    const tasks = d.tasks.filter((t: Task) => t.type === 'task')
    const done = tasks.filter((t: Task) => t.done).length
    const pct = tasks.length ? Math.round(done / tasks.length * 100) : 0
    const dateDisp = new Date(dateKey + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })
    return `<div class="archive-item" onclick="viewArchiveDay('${dateKey}')">
      <div class="archive-date">${dateDisp}</div>
      <div class="archive-stats">${tasks.length} tasks · ${d.tasks.filter((t: Task) => t.type === 'anchor').length} anchors</div>
...
function viewArchiveDay(dateKey: string) {
  ...
  if (!raw) return
  const d = JSON.parse(raw)
  const anchors = d.tasks.filter((t: Task) => t.type === 'anchor')
  const tasks = d.tasks.filter((t: Task) => t.type === 'task')
  const done = tasks.filter((t: Task) => t.done).length
  ...
  if (contentEl) contentEl.textContent =
    `ANCHORS\n${anchors.length ? anchors.map((a: Task) => `  • ${a.text}`).join('\n') : '  (none)'}\n\nTASKS\n${tasks.length ? tasks.map((t: Task) => `  ${t.done ? '✓' : '○'} ${t.text}`).join('\n') : '  (none)'}\n\nCOMPLETION: ${done} / ${tasks.length} tasks\n\nNOTES\n${d.notes || '  (none)'}`
  if (modal) modal.classList.add('open')
}

// ── SETTINGS ──
function loadSettings() {
  const s = JSON.parse(localStorage.getItem('daily_settings') || '{}')
  ...
  if (s.email && emailEl) emailEl.value = s.email
  if (s.phone && phoneEl) phoneEl.value = s.phone
  if (s.time && timeEl) timeEl.value = s.time
  if ((s.email || s.phone) && statusEl) {
    statusEl.textContent = '✓ Saved — ready for backend connection'
...
function saveSettings() {
  ...
  localStorage.setItem('daily_settings', JSON.stringify(s))
  if (statusEl) {
...
function checkMorningGate() {
  ...
  const yRaw = localStorage.getItem('daily_' + yKey)
  if (!yRaw) return
  const yData = JSON.parse(yRaw)
  const incomplete = (yData.tasks || []).filter((t: Task) => t.type === 'task' && !t.done)
...
function updateGateProgress() {
  ...
  if (!el) return
  if (decided === 0) {
    el.textContent = `${total} item${total !== 1 ? 's' : ''} to review`
  } else if (decided < total) {
    el.textContent = `${decided} of ${total} reviewed · ${kept} kept`
  } else {
    el.textContent = `${kept} carried forward · ${total - kept} released`
  }
}
...
function gateCommit() {
  ...
  if (yRaw) {
    const yData = JSON.parse(yRaw)
    const incomplete = (yData.tasks || []).filter((t: Task) => t.type === 'task' && !t.done)
    incomplete.forEach((item: Task) => {
      if (gateDecisions[item.id] === 'keep') {
        const alreadyThere = state.tasks.some(t => t.text === item.text)
        if (!alreadyThere) {
          state.tasks.push({ ...item, id: Date.now() + Math.random(), done: false, carriedFrom: yKey })
        }
      }
    })
    saveDay(state)
...
function closeMyDay() {
  ...
  if (btn) {
    btn.textContent = '✓ Day Closed'
    btn.className = 'close-day-btn done'
  }
...
function loadGoals(): Goal[] {
  return JSON.parse(localStorage.getItem('daily_goals') || '[]')
}
...
function addGoal() {
  ...
  goals.push({ id: Date.now(), text, icon: GOAL_ICONS[goals.length % GOAL_ICONS.length] })
  saveGoals(goals)
  renderGoals()
  input.value = ''
  input.focus()
...
export default function DailyApp() {
  ...
  return (
    <>
      ...
      {/* Archive tab content (hidden by default) */}
      <div className="tab-content" id="tab-archive">
        <div style={{ padding: '32px 40px' }}>
          ...
          <div className="archive-list" id="archiveList">
            <div className="empty-state">No past days yet. Complete a day and export to build your archive.</div>
          </div>
        </div>
      </div>
      ...
      <div className="tab-content" id="tab-settings">
        <div style={{ padding: '32px 40px', maxWidth: '560px' }}>
          ...
          <p style={{ fontSize: '12px', color: 'var(--ink-mid)', lineHeight: '1.7', marginBottom: '24px' }}>
            Set once, forget it. When the backend is live your contact info and wake time will be used to send your 6am daily link automatically.
          </p>
          <div className="notify-section">
            <div className="notify-label">Contact Info — stored locally</div>
